...
if __name__ == "__main__":
    ...
    # Find majority class for intent and attribute from training data
    intent_counter = Counter(train_intents)
    attr_counter = Counter(train_attrs)
    input_counter = Counter(train_inputs)
    
    majority_intent = intent_counter.most_common(1)[0][0]
    majority_attr = attr_counter.most_common(1)[0][0]
    majority_input = input_counter.most_common(1)[0][0]
    
    # Convert to indices
    majority_intent_idx = intent_encoder.transform([majority_intent])[0]
    majority_attr_idx = attr_encoder.transform([majority_attr])[0]
    ...
    print(f"Majority attribute: {majority_attr}")
    print(f"Majority input: {majority_input}")
    print()

    # Make majority class predictions (same for all test samples)
    preds_intent = np.full(len(test_texts), majority_intent_idx)
    preds_attr = np.full(len(test_texts), majority_attr_idx)
    preds_input = [majority_input] * len(test_texts)
