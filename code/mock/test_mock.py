...
if __name__ == "__main__":
    ...
    # Set up output file
    output_file = os.path.join(script_dir, "test_mock_output.txt")
    tee = Tee(output_file)
    sys.stdout = tee
    ...
    try:
        ...
        # Load model
        print(f"📦 Loading Mock Predictor...")
        ...
        for i, text in enumerate(test_texts):
            ...
            intent, slots = predictor.predict(text)
            mock_time = (time.perf_counter() - start_time) * 1000.0  # Convert to ms
            
            pred_intents.append(intent)
            pred_attrs.append(slots['attribute'])
            pred_entities.append(slots['input'])
            total_mock_time += mock_time
            
            # Check correctness
            intent_correct = intent == test_intents[i]
            attr_correct = slots['attribute'] == test_attrs[i]
            entity_correct = slots['input'].lower() == test_entities[i].lower()
            ...
            print(f"Extracted - Intent: {intent}, Attribute: {slots['attribute']}, Name: {slots['input']}")
        ...
        print(f"\n{'='*60}")
        print("Entity Extraction (Rule-based)")
        print(f"{'='*60}")
        print(f"Accuracy: {entity_accuracy:.4f} ({entity_matches}/{len(test_entities)})")
        
        # Timing summary
        print(f"\n⏱️  Timing Summary:")
        print(f"   Total Mock time: {total_mock_time:.2f} ms ({total_mock_time/1000:.2f} s)")
        print(f"   Average per example: {total_mock_time/len(test_texts):.2f} ms")
